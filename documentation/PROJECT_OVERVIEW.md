...
Give internal support and operations staff a single conversational interface — Slack — for looking up CPaaS platform data (virtual mobile numbers, gateways, tickets, customers, and source/line configuration) that lives in Google Sheets, without needing to open the spreadsheet, know which tab to search, or manually cross-reference records.
...
Full request path for a Slack deployment: **Slack → FastAPI → OpenAI (parse + optional answer generation) → Google Sheets (retrieval) → JSON response → Slack**, with n8n optionally relaying in place of the built-in adapter. See [ARCHITECTURE.md](./ARCHITECTURE.md) for the detailed breakdown of each layer.
...
- Slack as the target chat interface, via the built-in Events API adapter or n8n as an integration layer.
- Google Sheets as the live data backend option (CSV as the local/offline backend, used by the demo and the tests).
- Single-turn and short follow-up conversational memory, keyed by Slack thread (or user).
...
- A hardened, access-controlled web UI (the included React app is a demo/development interface — see below).
...
- **Slack interface** — via the built-in Events API adapter, or an n8n workflow that relays messages to the backend and replies in-thread.
- **API-key authentication** on the backend endpoint used by any client.
...
- **No authentication/authorization on *who* is asking within Slack** — any Slack user who can reach the bot could query any record; access is not scoped per user or channel.
- **The React frontend is a demo/development interface.** It exercises the same `/query` API as any other client and is the primary way to try the bot in this repo; a Slack deployment would not use it (see [ARCHITECTURE.md](./ARCHITECTURE.md)).
